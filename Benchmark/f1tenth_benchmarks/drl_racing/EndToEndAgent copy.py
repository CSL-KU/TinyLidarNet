...
class TinyAgent(BasePlanner):
    ...
    def transform_actor(self, obs):
        ...
        """
        speed = obs['vehicle_speed'] / self.planner_params.max_speed
        scan = np.clip(obs['scan'][::4] /self.planner_params.range_finder_scale, 0, 1)
        return scan

    def transform_critic(self, obs):
        """
        Transforms the observation received from the environment into a vector which can be used with a neural network.
...
class TrainTinyAgent(TinyAgent): 
    def __init__(self, map_name, test_id, extra_params={}):
        ...
        self.reward_generator = create_reward_function(self.planner_params, map_name)
        self.state = None
        self.nn_actor_state = None
        self.nn_critic_state = None
        self.nn_act = None
        self.action = None
        self.skip_n = int(np.ceil(1081 / self.planner_params.number_of_beams))
        self.state_space = self.planner_params.number_of_beams *2 + 1 
    ...
    def plan(self, obs): # This overwrites the above plan method
        actor_state = self.transform_actor(obs)
        critic_state = self.transform_critic(obs)
        
        self.add_memory_entry(obs, actor_state, critic_state)
        self.state = obs
        ...
        self.nn_actor_state = actor_state 
        self.nn_critic_state = critic_state 
        self.nn_act = self.agent.act(self.nn_actor_state)
        self.action = self.transform_action(self.nn_act)
    ...
    def add_memory_entry(self, s_prime, actor_state, critic_state):
        if self.nn_actor_state is not None:
            reward = self.reward_generator(s_prime, self.state, self.action)
            self.current_ep_reward += reward

            self.agent.replay_buffer.add(self.nn_actor_state, self.nn_critic_state, self.nn_act, actor_state, critic_state, reward, False)

    def done_callback(self, s_prime):
        """
        To be called when ep is done.
        """
        actor_state = self.transform_actor(s_prime)
        critic_state = self.transform_critic(s_prime)
        reward = self.reward_generator(s_prime, self.state, self.action)
        self.agent.replay_buffer.add(self.nn_actor_state, self.nn_critic_state, self.nn_act, actor_state, critic_state, reward, True)
